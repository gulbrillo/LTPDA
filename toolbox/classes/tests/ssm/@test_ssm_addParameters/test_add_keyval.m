% TEST_ADD_KEYVAL tests adding a parameter by key/value pair.
function res = test_add_keyval(varargin)
  ...
  sys2 = sys.addParameters('a', 1);
  
  % Modify by adding two parameters
  sys3 = copy(sys,1);
  sys3.addParameters('b', 2, 'c', 3);
  
  % Check with multiple inputs
  sys4 = addParameters(sys, sys, 'd', 4);
