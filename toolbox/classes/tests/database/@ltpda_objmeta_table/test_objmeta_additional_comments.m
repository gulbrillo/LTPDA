%
% DESCRIPTION: Tests that the metadata 'additional_comments' of a database
...
function varargout = test_objmeta_additional_comments(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    dbTable    = 'objmeta';
    tableField = 'additional_comments';
    
    % Check that the metadata was set with a submit PLIST
    try
      for nn = 1:numel(utp.objIds)
        
        % get 'additional_comments' from the objmeta table
        ...
        assert(strcmp(val{1}, utp.sinfoPlist.find(tableField)))
      end
    catch Me
      throw(Me);
    end
    ...
    try
      for nn = 1:numel(utp.objIdsStruct)
        ...
        assert(strcmp(val{1}, utp.sinfoStruct.(tableField)))
      end
    catch Me
      throw(Me);
    end
