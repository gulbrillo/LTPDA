%
% DESCRIPTION: Tests that the metadata 'keywords' of a database
...
function varargout = test_objmeta_keywords(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % get 'keywords' from the objmeta table
        ...
        assert(~isempty(strfind(val{1}, utp.sinfoPlist.find(tableField))))
      end
    catch Me
      throw(Me);
    end
    ...
    try
      for nn = 1:numel(utp.objIdsStruct)
        ...
        assert(~isempty(strfind(val{1}, utp.sinfoStruct.(tableField))))
      end
    catch Me
      throw(Me);
    end
