%
% DESCRIPTION: Tests that the metadata 'experiment_title' of a database
...
function varargout = test_objmeta_experiment_title(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    dbTable    = 'objmeta';
    tableField = 'experiment_title';
    
    % Check that the metadata was set with a submit PLIST
    try
      for nn = 1:numel(utp.objIds)
        
        % get 'experiment_title' from the objmeta table
