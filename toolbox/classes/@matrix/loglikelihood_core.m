...
%
% [LLH SNR LLHexp SNRexp LLH(f)] = loglikelihood(model,xn,in,out,noise,params, lp);
%
% It is implemented this way to achieve maximum in-loop speed of
% the Metropolis algorithm.
%
% INPUTS:   model     - The symbolic MATRIX system.
...
%        LLH(f) - The LLH as a function of frequency (cell array).elihood
...
function varargout = loglikelihood_core(varargin)
  ...
  for k = 1:Nexp
    ...
    for jj = 1:numel(model(k).objs)
      % evaluate models
      h(:,jj) = model(k).objs(jj).setParams(params,xn).double;
    end
