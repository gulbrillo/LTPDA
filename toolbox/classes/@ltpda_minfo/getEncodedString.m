...
function info = getEncodedString(obj)
  ...
  if numel(obj) > 1
    error('### It is not possible to encode more than one ltpda_minfo objects.');
  end
%   if ~isempty(obj.children)
%     error('### It is not possible to encode an ltpda_minfo object with children.');
%   end
  if ~isempty(obj.sets)
    warning('LTPDA:ENCODE_MINFO', '!!! Can not encode the sets of the ltpda_minfo object (mname: %s)', obj.mname);
  end
  if ~isempty(obj.plists)
    warning('LTPDA:ENCODE_MINFO', '!!! Can not encode the plists of the ltpda_minfo object (mname: %s)', obj.mname);
  end
