...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: MSUPTITLE Puts a title above all subplots.
...
% CALL:        xaxis(x1,x2)
%
% REMARK:      If the figure or axis units are non-default, this will break.
%
% HISTORY:     15-06-1995 Drea Thomas (drea@mathworks.com)
%                 Creation.
%              13-12-2000 John Cristion
%                 Modified.
%              13-03-2004 Mark Histed (histed@mit.edu)
%                 Fix disappearing legend on last plot
%              xx-xx-xxxx M Hewitson
%                 Just a copy and rename with some small adjustements
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function hout = msuptitle(str)
  ...
  h=[];
  for i=1:length(ch)
    if ~strcmp(get(ch(i), 'Tag'), 'LTPDA_ANNOTATION')
  ...
  if max_y > plotregion,
    ...
    for i=1:length(h),
      if ~strcmp(get(h(i),'Tag'),'legend') &&  ~strcmp(get(h(i),'Tag'),'LTPDA_ANNOTATION')
  ...
  legH = legend;
  if ~isempty(legH) && ~isa(legH, 'matlab.graphics.illustration.Legend')
    axes(legH);
  end
