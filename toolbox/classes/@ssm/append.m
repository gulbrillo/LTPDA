...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: append appends embedded subsytems, does not close any loop
%
% CALL: sys = ltpda_ss_append(sys_array)
%
% INPUTS: sys_array - array or list of systems to append
%
% OUTPUTS: sys - appended system
%
%  <a href="matlab:utils.helper.displayMethodInfo('ssm', 'append')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function   varargout = append( varargin )
  ...
  if Nsys < 2
    error('### Two or more systems are needed to append.');
  end
  
  % We want to force a copy - this is not a modify method
  if nargout ~= 1
    error('### append cannot be used as a modifier. Please give exactly one output variable.');
  end
  ...
  sys_out.inputs = [outputs inputs_ext];
  ...
  sys_out.amats = amats;
  sys_out.bmats = [B_in B_ext];
  sys_out.cmats = cmats;
  sys_out.dmats = [D_in, D_ext];
