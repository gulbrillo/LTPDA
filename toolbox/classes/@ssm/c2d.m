...
%
% <a href="matlab:utils.helper.displayMethodInfo('ssm', 'c2d')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = c2d(varargin)
  ...
  if ~isempty(opts)
    if ~isa(opts,'ltioptions.c2d')
      error('Options must be of type ltioptions.c2d, which can be created by calling c2dOptions');
    end
  end
  ...
  for ii = 1:numel(modsCopy)
    ...
    % apply MATLAB c2d method
    if isempty(opts)
      modss = c2d(modss,Ts,ssmethod);
    else
      modss = c2d(modss,Ts, opts);
    end
    ...
    % set Name
    mod.setName(sprintf('c2d[%s]',modsCopy(ii).name))
...
function pl = buildplist(set)
  ...
  p = param(...
    ...
    '<li>matched - matched pole-zero method (for SISO systems only)</li></ul>'...
    ]},...
    {1, {'ZOH', 'FOH','Impulse','Bilinear','matched'}, paramValue.SINGLE});
  pl.append(p);
  
  
  % Options
  p = param(...
    {'Options',['Fine-control options to be passed to the ss/c2d method.'...
    'Suitable options can be created with c2dOptions.']},...
    []);
  pl.append(p);
