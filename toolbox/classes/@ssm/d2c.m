...
%
% <a href="matlab:utils.helper.displayMethodInfo('ssm', 'd2c')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = d2c(varargin)
  ...
  if ~isempty(opts)
    if ~isa(opts,'ltioptions.d2c')
      error('Options must be of type ltioptions.d2c, which can be created by calling d2cOptions');
    end
  end
  ...
  for ii = 1:numel(modsCopy)
    ...
    % apply MATLAB d2c method
    if isempty(opts)
      modss = d2c(modss,ssmethod);
    else
      modss = d2c(modss,opts);
    end
    ...
    % set Name
    mod.setName(sprintf('d2c[%s]',modsCopy(ii).name))
...
function pl = buildplist(set)
  ...
  p = param(...
    ...
    {1, {'ZOH', 'Linear','Bilinear','matched'}, paramValue.SINGLE});
  pl.append(p);
  
  
  % Options
  p = param(...
    {'Options',['Fine-control options to be passed to the ss/d2c method.'...
    'Suitable options can be created with d2cOptions.']},...
    []);
  pl.append(p);
