...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: BANDPASS AOs containing time-series data. All other AOs with
%              no time-series data are skipped but appear in the output.
%              
% CALL:        b = bandpass(a, pl)      - use plist to get parameters
%              b = bandpass(a1, a2, pl) - bandpass both a1 and a2;
%                                           b is then a 2x1 vector.
%
% <a href="matlab:utils.helper.displayMethodInfo('ao', 'bandpass')">Parameters Description</a>
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function varargout = bandpass(varargin)
  ...
  for jj = 1:numel(bs)
    if isa(bs(jj).data, 'tsdata')
      ...
      % make filter
      inhist = bs(jj).hist;
      ff = miir.bandpass(bs(jj).fs, fc, order);
      ...
      if ~callerIsMethod
        % set name
        bs(jj).name = sprintf('bandpass(%s)', ao_invars{jj});
        % Add history
      ...
      warning('!!! BANDPASS only works on tsdata objects. Skipping AO %s', ao_invars{jj});
    end
  end
