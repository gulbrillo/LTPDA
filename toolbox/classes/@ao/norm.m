...
%
% <a href="matlab:utils.helper.displayMethodInfo('ao', 'norm')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function varargout = norm(varargin)
  ...
  % is the correct type and shape for doing norm.
...
function plout = buildplist(varargin)
  ...
  % Option
  p = param({'option', ['A string or value that can be submited, e.g. ''inf'' for<br>'...
    'the infinity norm of the input ao, the largest row<br>'...
    'sum. Options are the same as for MATLAB''s norm.']}, paramValue.EMPTY_STRING);
  plout.remove('option');
  plout.append(p);
end
