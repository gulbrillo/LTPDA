...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: IZTRANS implements continuous z-domain inverse Z-transform for smodel objects.
%
% CALL:        obj = iztrans(mdl, var)
%              obj = mdl.iztrans(var)
%              obj = mdl.iztrans(in_var, ret_var) [Please note the order!]
%              obj = mdl.iztrans(plist('ret_var', ret_var, 'in_var', in_var))
%              obj = mdl.iztrans(pl)
%
...
%
% <a href="matlab:utils.helper.displayMethodInfo('smodel', 'iztrans')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function varargout = iztrans(varargin)
  
  % Settings
  operatorName = 'iztrans';
...
function pl = buildplist()
  ...
  % Ret_Var
  p = param({'ret_var', 'Return variable, to transform into.'}, paramValue.STRING_VALUE('n'));
  pl.append(p);
