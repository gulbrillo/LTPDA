...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: LAPLACE implements continuous s-domain Laplace transform for smodel objects.
%
% CALL:        obj = laplace(mdl, ret_var)
%              obj = mdl.laplace(ret_var)
%              obj = mdl.laplace(in_var, ret_var) [Please note the order!]
%              obj = mdl.laplace(plist('ret_var', ret_var, 'in_var', in_var))
%              obj = mdl.laplace(pl)
%
...
%
% <a href="matlab:utils.helper.displayMethodInfo('smodel', 'laplace')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function pl = buildplist()
  ...
  % Ret_Var
  p = param({'ret_var', 'Return variable, to transform into.'}, paramValue.STRING_VALUE('s'));
  pl.append(p);
