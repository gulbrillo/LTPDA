...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: ZTRANS implements continuous z-domain Z-transform for smodel objects.
%
% CALL:        obj = ztrans(mdl, var)
%              obj = mdl.ztrans(var)
%              obj = mdl.ztrans(in_var, ret_var) [Please note the order!]
%              obj = mdl.ztrans(plist('ret_var', ret_var, 'in_var', in_var))
%              obj = mdl.ztrans(pl)
%
...
%
% <a href="matlab:utils.helper.displayMethodInfo('smodel', 'ztrans')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function pl = buildplist()
  ...
  % Ret_Var
  p = param({'ret_var', 'Return variable, to transform into.'}, paramValue.STRING_VALUE('z'));
  pl.append(p);
