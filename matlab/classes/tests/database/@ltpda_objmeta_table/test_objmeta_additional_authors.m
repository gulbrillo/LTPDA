%
% DESCRIPTION: Tests that the metadata 'additional_authors' of a database
...
function varargout = test_objmeta_additional_authors(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    dbTable    = 'objmeta';
    tableField = 'additional_authors';
    
    % Check that the metadata was set with a submit PLIST
    try
      for nn = 1:numel(utp.objIds)
        
        % get 'additional_authors' from the objmeta table
