%
% DESCRIPTION: Tests that the metadata 'name' of a database
...
function varargout = test_objmeta_name(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % get 'name' from the objmeta table
        ...
        % Check that the name in the meta table is the same as the name of
        % the test object.
        assert(strcmp(val{1}, utp.testData(nn).name))
      end
    catch Me
      throw(Me);
    end
