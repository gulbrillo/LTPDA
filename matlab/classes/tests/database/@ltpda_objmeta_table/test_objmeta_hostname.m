%
% DESCRIPTION: Tests that the metadata 'hostname' of a database
...
function varargout = test_objmeta_hostname(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % get 'hostname' from the objmeta table
        ...
        % Check that the hostname of the meta table is the same as the
        % current hostname.
        h = char(getHostName(java.net.InetAddress.getLocalHost));
        assert(strcmp(val{1}, h));
      end
    catch Me
      throw(Me);
    end
