%
% DESCRIPTION: Tests that the metadata 'analysis_desc' of a database
...
function varargout = test_objmeta_analysis_desc(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    dbTable    = 'objmeta';
    tableField = 'analysis_desc';
    
    % Check that the metadata was set with a submit PLIST
    try
      for nn = 1:numel(utp.objIds)
        
        % get 'analysis_desc' from the objmeta table
