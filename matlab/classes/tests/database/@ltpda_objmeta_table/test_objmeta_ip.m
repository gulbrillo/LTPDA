%
% DESCRIPTION: Tests that the metadata 'ip' of a database
...
function varargout = test_objmeta_ip(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % Check that in the meta table is the same as the current IP
        ip = char(getHostAddress(java.net.InetAddress.getLocalHost));
        assert(strcmp(val{1}, ip));
      end
    catch Me
      throw(Me);
    end
