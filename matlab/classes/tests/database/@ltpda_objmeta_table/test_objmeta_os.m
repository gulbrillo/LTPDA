%
% DESCRIPTION: Tests that the metadata 'os' of a database
...
function varargout = test_objmeta_os(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % Check that the Operating system is the same as the current
        % Operating system.
        prov = provenance();
        assert(strcmp(val{1}, prov.os))
      end
    catch Me
      throw(Me);
    end
