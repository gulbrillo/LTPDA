%
% DESCRIPTION: Tests the 'cdata' table of the database.
%
% CHECKS:      - Check that the 'yunits' contains the correct
%                yunits from the test data.
%

function varargout = test_cdata_yunits(varargin)
  ...
  if ~utp.testRunner.skipRepoTests()
    ...
    try
      for nn = 1:numel(utp.objIds)
        ...
        % Check that the entry in the table is the same as the yunits of
        % the test data
        assert(strcmp(val{1}, char(utp.testData(nn).yunits)))
