...
function varargout = update_struct(varargin)
  ...
  %%%%%%%%%%%%%%%%%%%%%%%   Update version '2.4' ->'2.5'  %%%%%%%%%%%%%%%%%%%%%%%
  
  if utils.helper.ver2num(struct_ver) < utils.helper.ver2num(tbx_ver) 
    
    struct_ver = '2.5.3';
    
    if isfield(obj_struct, 'objs')
      if ~isfield(obj_struct, 'names') || isempty(obj_struct.names)
        for ss = 1:numel(obj_struct)
          for kk=1:numel(obj_struct(ss).objs)
            obj_struct(ss).names{kk} = sprintf('obj%d', kk);
          end
        end
      end
    end
