...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: UNPACK unpacks the objects in a collection and sets them to the
...
% dummy output variable. For example, suppose we have a collection object
...
%
% <a href="matlab:utils.helper.displayMethodInfo('collection', 'unpack')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = unpack(varargin)
  ...
  if numel(ms) ~= 1
    error('collection/unpack can only work on a single collection object');
  end
  
  if nargout ~= numel(ms.objs)
    error('The number of outputs doesn''t match the number of objects in the collection.');
  end
...
function ii = getInfo(varargin)
  ...
  % Build info object
  ii = ltpda_minfo(mfilename, mfilename('class'), 'ltpda', utils.const.categories.output, '', sets, pls);
  ii.setArgsmin(2);
