...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: SETOUNITS sets the 'ounits' property of each filter-object
...
%
% <a href="matlab:utils.helper.displayMethodInfo('filterbank', 'setOunits')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function varargout = setOunits(varargin)
  ...
  for j=1:numel(fb)
    if strcmpi(fb(j).type, 'parallel')
      fb(j).filters.setOunits(values{j});
      if ~callerIsMethod
        plh = pls.pset(pName, values{j});
        fb(j).addHistory(getInfo('None'), plh, fb_invars(j), fb(j).hist);
      end
    else
      warning('LTPDA:FILTERBANK:SETOUNITS', '!!! Skip filterbank %s because it is not a parallel filterbank.', fb(j).name);
    end
  end
