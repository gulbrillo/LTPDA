...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: ctranspose implements conjugate transpose operator for a matrix object.
%
% CALL:        obj = obj'
%              obj = ctranspose(obj);
%
% <a href="matlab:utils.helper.displayMethodInfo('matrix', 'ctranspose')">Parameters Description</a>
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function varargout = ctranspose(varargin)
  ...
  for jj = 1:numel(bs) 
    ...
    for ii = 1:s1
      for kk = 1:s2
        trp(kk,ii) = conj(bs(jj).objs(ii, kk));
      end
    end
    bs(jj).objs = trp;

    % set Name
    bs(jj).name = sprintf('ctranspose(%s)', as(jj).name);
    % add history
