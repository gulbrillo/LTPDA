...
- **`sshd`** — OpenSSH server on port 22 (mapped to host port 2222). Accepts only password
  ...
  to `db:3306`.
- **`ssh_sync_daemon.py`** — Flask HTTP API on port 9922 (internal Docker network only — not
  published to the host). The FastAPI backend calls this daemon whenever a user is created, has
  their password changed, or is deleted — keeping Linux accounts inside the container in sync with
  the web UI. All requests are signed with HMAC-SHA256.
...
**Account persistence:** user accounts are persisted to `config/ssh_users.json` (in the shared
volume). On every container start, the daemon restores any accounts that are missing from the
container filesystem — so accounts survive `docker compose up --build` rebuilds automatically.

**Daemon log:** the sync daemon writes to `config/sshgateway.log` on the host (shared via the
`./config` volume mount). The last 60 lines are also readable from **Admin → Settings** when the
daemon is unreachable. On the server:

```bash
tail -f config/sshgateway.log
```

**Startup order:** the daemon waits for `config/config.json` to be written by the first-run setup
wizard before it begins accepting requests. If the container starts before setup has been run, the
daemon simply loops in the log until the file appears.
...
This forwards your local port 13306 → SSH gateway container → MySQL container.
...
In **LTPDAprefs**, set:
- **Hostname** — `localhost`
- **Port** — `13306` (matches the local port in the `ssh -L` command above)
...
|---------|-------|
| Hostname | `localhost` |
| Port | `13306` (or whatever local port you chose in the ssh command) |
...
│   └── ssh_sync_daemon.py  Flask API on port 9922 (internal only); HMAC-signed; manages Linux accounts; persists to ssh_users.json
├── config/                 Runtime config volume (excluded from git)
│   ├── config.json         Generated by setup wizard; shared with sshgateway container
│   ├── ssh_users.json      Persistent SSH account store (username → pw hash); restored on container restart
│   ├── ssh_host_keys/      SSH host keys (generated on first start; persist across rebuilds)
│   └── sshgateway.log      SSH sync daemon log (written by sshgateway, readable on host)
